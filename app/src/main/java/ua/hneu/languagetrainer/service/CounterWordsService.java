...
public class CounterWordsService {
	...
	public static CounterWordsDictionary getCounterwordsBySection(String section,
        ...
        if(section.isEmpty()){
            if (App.lang == Languages.RUS)
                c = cr.query(CounterWordsDAO.CONTENT_URI, col, null, null,
                        null, null);
            else
	...
	public static CounterWordsDictionary createCurrentDictionary(String section,
        ...
        Log.d("CWD","createCurrentDictionary numberWordsInCurrentDict="+numberWordsInCurrentDict+", section="+section);
        if (App.allCounterWordsDictionary == null) {
            App.allCounterWordsDictionary = new CounterWordsDictionary();
            App.allCounterWordsDictionary = getCounterwordsBySection("", App.cr);
        }
        CounterWordsDictionary currentDict = new CounterWordsDictionary();
        // if words have never been showed - set entries randomly
        if (App.userInfo.getIsCWLaunchedFirstTime() == 1) {
            currentDict.getEntries().addAll(
                    App.allCounterWordsDictionary.getRandomEntries(
    ...
    public void makeProgress(ContentResolver cr) {
        Random r = new Random();
        int i=0;
        for (CounterWord e : App.allCounterWordsDictionary.getEntries()) {
        ...
        App.userInfo.setLearnedCounterWords(i);
